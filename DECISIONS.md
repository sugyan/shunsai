...
- **The 0.1.0 changelog entry is hand-written, and later ones are not.** Both jobs fire on the same push to `main`, so on the first one the `release` job would publish 0.1.0 before the release PR that creates `CHANGELOG.md` could be merged — and a missing `include` entry is skipped silently rather than failing, so the first tarball would have shipped without the file the manifest names. Writing it up front also beats what git-cliff would make of 29 commits of initial development. **Its date must match the day of the actual publish.**
...
- **Still open: trusted publishing.** It would remove `CARGO_REGISTRY_TOKEN` entirely, but crates.io configures a trusted publisher against a crate that already exists, so it cannot cover the first upload. Worth taking once 0.1.0 is up.
- **Prerequisites that are not in the tree**, and that the workflow cannot check for itself: the `CARGO_REGISTRY_TOKEN` and `RELEASE_PLZ_TOKEN` secrets. The repository had **no secrets at all** when this landed, so both are new, and the first push to `main` after this merges is what exercises them.

...
- **Rejected: the `prose-discipline` plugin.** Its marketplace is a local directory with no remote, so a cloud checkout resolved none of it — skill, reviewer agent, `/prose-check`, `/create-pr` and hook alike, silently. Run over this whole tree its mechanical checker returns **two hits, both false positives**: CLAUDE.md's own kept figures, which the rule it enforces permits in a document and bans in a code comment. Zero true positives. rinsai measured the other half independently — its review loop ran ten passes on one branch, **five of which changed no executable line**, against a checker that found nothing.
- **Rejected: publishing that marketplace to GitHub.** It fixes the cloud half, and keeps the loop that is the cost. One shared copy for three repositories is the argument for it and does not outweigh a review step with no stopping condition.
- **Rejected: a CI job for either hook, and this one is settled rather than deferred.** These checks exist because an *agent* is careless in ways someone writing this by hand is not; the repository's CI is the project's, and a machine's own discipline is the wrong thing to put in a job every contributor pays for. **A check that only matters because Claude Code is editing lives in a hook and nowhere else.** Both scripts take `--all` so a CI step remains one line away if that ever changes.
- **What the in-repo hooks are worth, measured rather than assumed.** Before the comment rule was written, three `.rs` comments carried a measured figure and one carried an instruction the code cannot enforce; none has since, across 16 commits. The history-narration check has **never fired**, before or after — it is kept because a grep is free, not because anything says it works. `corpus-path.sh` has never fired either, and by construction cannot on a checkout without the corpus.
- ⚠️ **What a hook costs, stated rather than glossed.** It fires only when Claude Code does the editing. A hand-written commit, another tool and CI see none of it, and nothing obliges anyone to run `--all`.
- **Reopens if** a false claim reaches `main` that one of the dropped checks would have caught. The plugin's checker read `.md` files too, and that is the half given up here.
- **Deferred: compressing the documents.** DECISIONS.md is the size rinsai's was when it replaced its log with a question-indexed FAQ, and a cloud session carries no memory outside the repository, so the documents are all it has. Not folded in here: the review is mechanical for the setup and per-sentence for the log, and shunsai has a fourth document — BENCHMARKS.md owns the measurement method — so rinsai's five-file shape does not transfer unchanged.

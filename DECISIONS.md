...
- ⏳ **Measured 2026-08-12 and deliberately not recorded — the machine could not resolve an effect this size.** A paired A/B of both binaries, three order-reversed passes each, failed two of BENCHMARKS.md's three conditions: 19 of 46 ids exceeded σ = 4 % in some pass, 20 of 46 disagreed by more than 4 % across the three passes of *one* binary, and the controls — ids the change provably cannot reach — themselves read 0.7 % apart between the two binaries with ±3 % scatter run to run, non-monotonic, so it is noise rather than a trend that could be subtracted. **No `benches/history` entry was written; nothing here is a figure.** What the run does establish is that `perft/*` sits inside the control band, so a large regression is excluded. What it leaves open is narrower and specific: `do_undo/games-v1` and the two `movegen/*-buf` ids read high with no mechanism that explains them — the `Position` copied by `with_drop` got *smaller*, not larger — and `movegen/maxmoves-buf` is the id the 2026-08-07 run already named as the largest single mover in either direction. **Those three are what a quiet machine should re-measure**, and they are the reason this entry claims the type rather than a speed.
...
**`Position` does not derive `Copy`**, though every field now would allow it. Roughly 380 bytes is small enough to copy deliberately and large enough that copying it *by accident* in a search's hot loop would not announce itself; `.clone()` at a copy-make site puts the cost in the source, where a reader will meet it. Adding `Copy` later is a break, so this is the window's decision rather than a default — and it is the one item here most likely to be revisited, because copy-make is exactly the caller that would want it.

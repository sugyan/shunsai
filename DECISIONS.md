...
- **`cargo-semver-checks` has no baseline at 0.1.0** — there is no published version to compare against, so it first does real work at 0.1.1. That is also when `rinsai` starts consuming releases, so the two line up; it is not a gap to close before publishing.
...
- **The one prerequisite that is a secret is `RELEASE_PLZ_TOKEN`.** The repository had **no secrets at all** when this was written; the token was added the same day.

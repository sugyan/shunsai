...
- **crates.io authentication is Trusted Publishing, so there is no registry secret at all.** release-plz mints a short-lived token from the job's OIDC identity — `id-token: write`, no `CARGO_REGISTRY_TOKEN`, and specifically *not* the `rust-lang/crates-io-auth-action` step, whose API calls release-plz implements itself. **The cost is that v0.1.0 must be published by hand**: crates.io only accepts a trusted publisher configuration against a crate that already exists, so the first upload cannot use the mechanism that secures every later one. That is a one-time exception, written down in [RELEASING.md](./RELEASING.md) rather than left as folklore.
- ⚠️ **Two parts of that configuration live on crates.io, are invisible from this repository, and fail silently.** The workflow **file name** is matched, so renaming `release-plz.yml` stops publishing; and the configuration's environment field must stay empty because the release job declares no environment. This is the whole reason RELEASING.md exists as a document rather than a paragraph — the settings it records cannot be checked from the tree, so nothing here would catch them drifting.
- **The one prerequisite that is a secret is `RELEASE_PLZ_TOKEN`.** The repository had **no secrets at all** when this landed, and the first push to `main` after it merges is what exercises it.

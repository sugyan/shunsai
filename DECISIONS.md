...
- **The `--all-features` trap generalizes past measurement** (BENCHMARKS.md, rewritten). `slider-naive` wins the backend priority order, so clippy had only ever linted the oracle's cfg configuration and rustdoc only documented it. Both now run twice, once each way. Making the doc run possible needed `sliders.rs`'s intra-doc link to `naive` dropped — the module is cfg-gated, so the link cannot resolve in the configuration docs.rs renders.

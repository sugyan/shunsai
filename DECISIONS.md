...
- **Unblocks copy-make (open question 10)**, which was described as needing a `Position` redesign and is now a driver change.
- **It costs `do_undo/games-v1` about 4 %, and that is the whole of the durable cost** (measured 2026-08-12, five order-alternating passes per binary: every head reading above every base reading, controls at −1.5..−0.05 %). The `Undo` round-trips through the caller where it used to be written once into the position's own `Vec`. That is the price of the shape, and it gives open question 9 a reason to be taken.
